...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="XeDesk", help="The app name."
    )
    parser.add_argument(
...
def replace_app_name_in_langs(app_name):
    ...
    for file_path in langs_dir.glob("*.wxl"):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            lines[i] = line.replace("XeDesk", app_name)
...
def update_license_file(app_name):
    ...
    license_content = license_content.replace("website www.fuxiaoer.top and other ", "")
    license_content = license_content.replace("XeDesk", app_name)
